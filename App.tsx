...
const FILTER_CATEGORIES: FilterCategory[] = [
  {
    ...
    filters: [
      ...
      { id: 'lego', name: 'LEGO Minifigure', prompt: 'Transform into official LEGO minifigure style. CHARACTER - Classic LEGO minifigure proportions with cylindrical head, C-shaped hands, and blocky body. Yellow skin tone (classic LEGO style). Simple dot eyes or printed face details. Plastic shiny surface texture. DETAILS - Hair piece as separate molded element. Clothing printed on torso piece. Accessories as LEGO elements. ENVIRONMENT - LEGO brick world with studs visible. Plastic toy aesthetic throughout. LIGHTING - Clean bright toy photography lighting. STYLE - Official LEGO set box art quality. The LEGO Movie aesthetic. Keep subject recognizable as their LEGO minifigure version.' },
      { id: 'cyberpunkneon', name: 'Cyberpunk Neon', prompt: 'Transform into intense cyberpunk neon art style. COLORS - Vibrant neon pink, electric blue, hot magenta, cyber purple against deep blacks. Glowing neon edges and highlights. LIGHTING - Dramatic neon rim lighting. Multiple colored light sources. Neon signs reflecting on surfaces. Rain-slicked streets with neon reflections. ATMOSPHERE - Dark dystopian urban night. Smoke and haze catching neon light. High contrast between darks and neon brights. STYLE - Blade Runner, Akira, cyberpunk anime influence. Gritty futuristic aesthetic. Tech noir mood. EFFECTS - Chromatic aberration, lens flares from neon, glitch elements. Keep subject recognizable with intense cyberpunk neon treatment.' },
    ],
  },
  {
  ...
  },
  {
    ...
    filters: [
      ...
      { id: 'dccomic', name: 'DC Comics', prompt: 'Transform into DC Comics superhero illustration style with DRAMATIC visual transformation. LINEWORK - Bold dynamic ink lines with Jim Lee influence. Strong black spotting and crosshatching. COLORS - Deep rich jewel tones. Iconic DC palette: deep blues, bold reds, bright yellows, rich greens. High saturation comic book colors. SHADING - Heavy dramatic shadows with strong contrast. Chiaroscuro lighting like Alex Ross paintings. ANATOMY - Heroic idealized proportions. Powerful muscular definition. Dynamic action poses. COSTUME - Add superhero costume elements: cape, emblem, hero suit design appropriate to subject. BACKGROUND - Comic book action background with speed lines, energy effects, or Gotham/Metropolis cityscape. STYLE - Bronze Age DC Comics meets modern Jim Lee. Batman, Superman, Wonder Woman artistic quality. The subject should look like they belong in Justice League. CRITICAL - Make transformation OBVIOUS and dramatic, not subtle.' },
...
// Track prompt usage for authenticated users
  ...
  const handleApplyCustomStyle = async (styleImageUrl: string, styleDescription: string) => {
    ...
    try {
      ...
      // Build a custom style prompt - Image 1 is style reference, Image 2 is target to transform
      const customPrompt = `You are given two images:
- IMAGE 1 (first image): This is the STYLE REFERENCE - analyze its artistic style, colors, lighting, and visual aesthetic
- IMAGE 2 (second image): This is the TARGET PHOTO - transform this image to match the style from Image 1

${styleDescription ? `USER NOTES: ${styleDescription}\n\n` : ''}STYLE TRANSFER TASK:
1. Study the artistic style, color palette, lighting, textures, and visual techniques from IMAGE 1
2. Apply those exact visual characteristics to IMAGE 2
3. The output should show the person from IMAGE 2, but rendered in the style of IMAGE 1
4. Preserve the exact facial features, identity, pose, and composition from IMAGE 2
5. Only change the artistic rendering style to match IMAGE 1

CRITICAL: The final image must show the person from IMAGE 2 (the target photo), NOT the person from IMAGE 1.`;
